...
res =  maximum $ foldl1 go triangle

go :: [Int] -> [Int] -> [Int]
go [] _       = []
go _ []       = []
go x y = refine $ zipWith (+) (repeater x) ([head y] ++ (repeater (take (length y - 2) (tail y))) ++ [last y])  

repeater [] = []
repeater (x:xs) = x : x : repeater xs

refine :: [Int] -> [Int]
refine (x:xs) = x : refine' xs
  where
    refine' [] = []
    refine' [x] = [x]
    refine' (x:xs:xss) = max x xs : refine' xss

-- data --

triagList :: [a] -> [[a]]
triagList x = triagList' x 1
	
triagList' :: [a] -> Int -> [[a]]
triagList' [] _ = []
triagList' x n  = (take n x) : (triagList' (drop n x) (n+1))

triangle = triagList tri
